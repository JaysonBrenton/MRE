...
purpose:
  Defines the mandatory User Experience principles for My Race Engineer (MRE).
  These rules apply to all version 0.1.1 features and will remain the baseline
  foundation for Beta and Production. Ensures every screen behaves consistently,
  LLM contributors follow predictable UX patterns, and version 0.1.1 UI remains
  simple, desktop-optimized, and enterprise-grade. This document is fully
  authoritative. No UX deviations are allowed in version 0.1.1.
relatedFiles:
...
This document defines the mandatory User Experience principles for **My Race
Engineer (MRE)**. These rules apply to **all version 0.1.1 features** and will
remain the baseline foundation for Beta and Production.
...
This document is fully authoritative. No UX deviations are allowed in version
0.1.1.
...
- See `docs/design/navigation-patterns.md` for detailed sidebar implementation
  guidelines
...
Vertical spacing must follow the spacing scale defined in the dark theme
guidelines.
...
All buttons in MRE must use the **standard outlined/secondary style** for
consistency. This style provides a clean, professional appearance that aligns
with the dark theme guidelines.
...
- **Full-width buttons**: Use `w-full` for form submit buttons and primary
  actions where appropriate
- **Auto-width buttons**: Omit `w-full` for navigation buttons and secondary
  actions that should size to content
...
**Note:** Side navigation (sidebars) and tabs are allowed per
ADR-20251228-allow-sidebar-navigation and feature scope. See section 3.1 for
...
The following roles have specific responsibilities for implementing and
maintaining these UX principles:

- **Senior UI/UX Expert** (`docs/roles/senior-ui-ux-expert.md`): Primary owner
  of UX principles. Establishes and evolves UX principles, design patterns, and
  component guidelines. Ensures WCAG 2.1 AA compliance, defines information
  architecture, and guides design decisions for data visualization. Reviews
  UI/UX implementations for consistency and usability.

- **Next.js Front-End Engineer** (`docs/roles/nextjs-front-end-engineer.md`):
  Implements UX principles in code. Ensures UI components follow documented UX
  patterns, maintains design token usage, implements error boundaries with
  user-facing recovery paths, and upholds performance budgets. Works closely
  with Senior UI/UX Expert to translate design principles into implementation.

- **Quality & Automation Engineer**
  (`docs/roles/quality-automation-engineer.md`): Ensures UX principles are
  validated through automated testing. Implements accessibility checks,
  validates mobile breakpoints, and maintains test coverage for UX requirements.

- **Documentation & Knowledge Steward**
  (`docs/roles/documentation-knowledge-steward.md`): Maintains UX documentation,
  ensures UX principles stay current, and facilitates updates when UX patterns
  evolve.
