...
purpose: Defines command-line tools for administrators to manage LiveRC ingestion,
...
LiveRC ingestion, verify catalogue consistency, and run scheduled ingestion
jobs via cron. These tools operate entirely on the backend, outside the UI,
and represent the authoritative operational interface for ingestion control.
...
This ensures clarity when future connectors are added (e.g., mre ingest liverc, mre ingest another_source).
...
See `docs/operations/liverc-operations-guide.md` for complete execution instructions.
...
Description:
List all tracks currently stored in the Track catalogue.
...
Description:
Re-scrape the global LiveRC track list and update the Track table.
...
Description:
List events for a given track, optionally filtered by dates.
...
Description:
Populate or refresh events for a specific track.
...
Description:
Show a summary of ingestion subsystem health.
...
Description:
Perform checks across data tables to flag:
- Orphaned races
...
- Adding additional connectors later (e.g., elapsed-time sources, MyLaps, custom telemetry)
