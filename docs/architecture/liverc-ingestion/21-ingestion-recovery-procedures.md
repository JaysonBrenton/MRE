...
purpose: Defines end-to-end recovery processes for the LiveRC ingestion subsystem when
...
This document defines the end-to-end recovery processes for the LiveRC ingestion subsystem in My Race Engineer (MRE). Recovery procedures are critical for maintaining ingestion stability when failures occur due to upstream HTML changes, network faults, database inconsistencies, connector errors, or internal logic breakdowns.
...
These procedures ensure ingestion is resilient, predictable, and maintainable even under adverse conditions.

---

## 1. Principles of Recovery

1. The system must **never leave partially-ingested data in a state that appears complete**.
...
No partial race, result, or lap data is considered authoritative until ingestion completes.
...
Because ingestion is idempotent and reconstructable, full database restores should rarely be needed.
