 ...
 */
export async function findCarProfileById(id: string, userId: string): Promise<CarProfile | null> {
 ...
 */
export async function createCarProfile(data: Prisma.CarProfileCreateInput): Promise<CarProfile> {
 ...
 */
export async function deleteCarProfile(id: string, userId: string): Promise<CarProfile | null> {
  // First verify ownership
  const existing = await findCarProfileById(id, userId)
  if (!existing) {
    return null
