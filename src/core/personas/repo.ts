...
export async function getUserPersona(userId: string): Promise<Persona | null> {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Prisma client persona model is not available. This usually means the Prisma client needs to be regenerated or the dev server needs to be restarted. Try running: npx prisma generate and restart the dev server. Original error: ${error.message}`
      )
    }
    throw error
  }
