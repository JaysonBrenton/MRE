...
test.describe('Practice Day Search', () => {
  ...
  test('should render practice day search without toLocaleString errors', async ({ page }) => {
    ...
    const practiceDayButton = page.getByRole('button', { name: /practice.*day/i }).or(
      page.getByRole('tab', { name: /practice.*day/i })
    ).first()
    ...
    const trackSelect = page.getByLabel(/track/i).or(
    ...
    const yearInput = page.getByLabel(/year/i).or(
      page.locator('input[type="number"][placeholder*="year" i], input[type="number"][name*="year" i]').first()
    )
    
    const monthInput = page.getByLabel(/month/i).or(
      page.locator('input[type="number"][placeholder*="month" i], input[type="number"][name*="month" i]').first()
    )
    ...
    const toLocaleStringErrors = errors.filter(err => 
  ...
  test('should display practice day results when search is performed', async ({ page }) => {
    ...
    if (hasResults) {
      ...
      const totalLapsValue = page.locator('text=/Total Laps:/i').locator('..').getByText(/\d+/).first()
