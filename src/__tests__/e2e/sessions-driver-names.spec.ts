...
test.describe('Sessions Table Driver Names', () => {
  ...
  test('should display driver names in expanded sessions table', async ({ page }) => {
    ...
    const sessionsTable = page.locator('table').filter({ hasText: /session name|race overview/i }).first()
    ...
    const expandedTable = page.locator('table').filter({ 
    ...
    const validDriverNames = driverNames.filter(name => name.length > 0 && name !== '—' && name !== 'Unknown Driver')
    
    if (validDriverNames.length === 0) {
      // Take screenshot for debugging
      await page.screenshot({ path: 'test-results/sessions-table-no-driver-names.png', fullPage: true })
