...
export async function GET() {
  try {
    ...
    if (!persona) {
      return errorResponse("PERSONA_NOT_FOUND", "No persona assigned to user", undefined, 404)
    }
    ...
    const responseData: {
      ...
      participationDetails?: Awaited<
        ReturnType<typeof discoverDriverEvents>
    ...
    return errorResponse("INTERNAL_ERROR", "Failed to fetch user persona", undefined, 500)
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!personaId || typeof personaId !== "string") {
      return errorResponse("VALIDATION_ERROR", "personaId is required", undefined, 400)
    }
    ...
    if (!persona) {
      return errorResponse("PERSONA_NOT_FOUND", "Persona not found", undefined, 404)
    }
    ...
    return errorResponse("INTERNAL_ERROR", "Failed to assign persona", undefined, 500)
  }
}
