...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return successResponse({ profiles }, 200, "Driver profiles retrieved successfully")
  } catch (error: unknown) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Parse request body
    const bodyResult = await parseRequestBody<Omit<CreateDriverProfileInput, "userId">>(request)
    ...
    return successResponse({ profile }, 201, "Driver profile created successfully")
  } catch (error: unknown) {
    const errorInfo = handleApiError(error, request, requestId)
    return errorResponse(errorInfo.code, errorInfo.message, undefined, errorInfo.statusCode)
  }
