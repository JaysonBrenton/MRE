...
export default function ComparisonTest({
  ...
  const winners = useMemo(() => {
    ...
    const fastest = selectedDriverResults.reduce<WinnerInfo | null>((best, driver) => {
      ...
      if (!best || driver.fastLapTime < best.value) {
        return { driverId: driver.driverId, driverName: driver.driverName, value: driver.fastLapTime }
      }
      return best
    }, null)
    ...
    const mostConsistent = selectedDriverResults.reduce<WinnerInfo | null>((best, driver) => {
      ...
      if (!best || (driver.consistency > best.value && driver.consistency > 0)) {
        return { driverId: driver.driverId, driverName: driver.driverName, value: driver.consistency }
      }
      return best
    }, null)
    ...
    const bestAverage = selectedDriverResults.reduce<WinnerInfo | null>((best, driver) => {
      ...
      if (!best || driver.avgLapTime < best.value) {
        return { driverId: driver.driverId, driverName: driver.driverName, value: driver.avgLapTime }
      }
      return best
    }, null)
    ...
    const bestPosition = selectedDriverResults.reduce<WinnerInfo | null>((best, driver) => {
      if (!best || driver.positionFinal < best.value) {
        return { driverId: driver.driverId, driverName: driver.driverName, value: driver.positionFinal }
      }
      return best
    }, null)
  ...
  const getRanking = (driverId: string, metric: "fastest" | "mostConsistent" | "bestAverage" | "bestPosition"): number => {
  ...
  const normalizeValue = (value: number | null, metric: "fastest" | "mostConsistent" | "bestAverage" | "bestPosition"): number => {
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[var(--token-text-secondary)]">Fastest Lap</span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${getBadgeColor(fastestRank)}`}>
                          {fastestRank === 1 ? "🥇" : fastestRank === 2 ? "🥈" : fastestRank === 3 ? "🥉" : `#${fastestRank}`}
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[var(--token-text-secondary)]">Average Lap</span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${getBadgeColor(averageRank)}`}>
                          {averageRank === 1 ? "🥇" : averageRank === 2 ? "🥈" : averageRank === 3 ? "🥉" : `#${averageRank}`}
                        </span>
                      </div>
                      <div className="text-sm font-medium text-[var(--token-text-primary)]">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[var(--token-text-secondary)]">Consistency</span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${getBadgeColor(consistentRank)}`}>
                          {consistentRank === 1 ? "🥇" : consistentRank === 2 ? "🥈" : consistentRank === 3 ? "🥉" : `#${consistentRank}`}
                        </span>
                      </div>
                      <div className="text-sm font-medium text-[var(--token-text-primary)]">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[var(--token-text-secondary)]">Final Position</span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${getBadgeColor(positionRank)}`}>
                          {positionRank === 1 ? "🥇" : positionRank === 2 ? "🥈" : positionRank === 3 ? "🥉" : `#${positionRank}`}
                        </span>
                      </div>
                      <div className="text-sm font-medium text-[var(--token-text-primary)]">
                    ...
                    {/* Laps Completed */}
                    <div>
                      <span className="text-xs text-[var(--token-text-secondary)]">Laps Completed</span>
                      <div className="text-sm font-medium text-[var(--token-text-primary)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {winners.fastest && (
                <div className="p-3 rounded border border-[var(--token-border-default)] bg-[var(--token-surface)]">
                  <div className="text-xs text-[var(--token-text-secondary)] mb-1">Fastest Driver</div>
              ...
              {winners.mostConsistent && (
                <div className="p-3 rounded border border-[var(--token-border-default)] bg-[var(--token-surface)]">
                  <div className="text-xs text-[var(--token-text-secondary)] mb-1">Most Consistent</div>
                  <div className="text-sm font-semibold text-[var(--token-text-primary)] mb-1">
              ...
              {winners.bestAverage && (
                <div className="p-3 rounded border border-[var(--token-border-default)] bg-[var(--token-surface)]">
                  <div className="text-xs text-[var(--token-text-secondary)] mb-1">Best Average Lap</div>
                  <div className="text-sm font-semibold text-[var(--token-text-primary)] mb-1">
              ...
              {winners.bestPosition && (
                <div className="p-3 rounded border border-[var(--token-border-default)] bg-[var(--token-surface)]">
                  <div className="text-xs text-[var(--token-text-secondary)] mb-1">Best Position</div>
                  <div className="text-sm font-semibold text-[var(--token-text-primary)] mb-1">
