...
export default function LapDataTable({
  ...
  useEffect(() => {
    const fetchLapData = async () => {
      ...
      try {
        ...
        if (!result.success) {
          const errorMessage = result.error?.message || result.error?.details || "Failed to fetch lap data"
        ...
        const errorMessage = err instanceof Error ? err.message : "An error occurred while fetching lap data"
  ...
  const filteredAndSortedDrivers = useMemo(() => {
    ...
    if (driverFilter && driverFilter.trim() !== "") {
      ...
      filtered = filtered.filter((driver) =>
        driver.driverName.toLowerCase().includes(filterLower)
      )
    }
  ...
  const toggleRaceView = useCallback((raceId: string) => {
  ...
  if (loading) {
    return (
      <ChartContainer
        title="Lap Data"
        className={className}
        aria-label="Lap data table - loading"
      >
  ...
  if (error) {
    return (
      <ChartContainer
        title="Lap Data"
        className={className}
        aria-label="Lap data table - error"
      >
  ...
  if (lapData.length === 0) {
    return (
      <ChartContainer
        title="Lap Data"
        className={className}
        aria-label="Lap data table - no data"
      >
        <div className="flex items-center justify-center h-64 text-[var(--token-text-secondary)]">
          {selectedClass
                                      ...
                                      <table className="w-full min-w-[800px]">
                                        ...
                                        <tbody>
                                          {race.laps.map((lap) => {
                                            ...
                                            const isBest = isBestLap(lap.lapTimeSeconds, race.bestLapTime)
                                              ...
                                              >
                                                ...
                                                <td className="px-3 py-2 text-sm font-mono">
                                                  {delta !== null ? (
                                                    ...
                                                    <span className="text-[var(--token-text-secondary)]">—</span>
                                                  )}
                                                </td>
                                              </tr>
                                            )
                                      ...
                                      <table className="w-full min-w-[800px]">
                                        ...
                                        <tbody>
                                          {driverRace.laps.map((lap) => {
                                            const delta = getTimeDelta(lap.lapTimeSeconds, driverRace.bestLapTime)
                                            const isBest = isBestLap(lap.lapTimeSeconds, driverRace.bestLapTime)
