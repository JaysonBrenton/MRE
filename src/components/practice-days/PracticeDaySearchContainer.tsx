...
import ErrorDisplay from "../event-search/ErrorDisplay"
...
export default function PracticeDaySearchContainer({
  ...
  const [ingestedPracticeDays, setIngestedPracticeDays] = useState<Map<string, IngestedPracticeDay>>(new Map())
  ...
  useEffect(() => {
    ...
    const fetchPracticeDays = async () => {
      ...
      try {
        ...
        const discoverData = await parseApiResponse<{ practiceDays: PracticeDaySummary[] }>(discoverResponse)
        ...
        if (discoverData.success) {
          ...
          const practiceDaysList = 
            ...
            discoverData.data?.practice_days || 
          ...
          const transformedPracticeDays = practiceDaysList.map((pd: any) => {
            ...
            const sessionCount = typeof pd.session_count === 'number' ? pd.session_count : (typeof pd.sessionCount === 'number' ? pd.sessionCount : 0)
            const totalLaps = typeof pd.total_laps === 'number' ? pd.total_laps : (typeof pd.totalLaps === 'number' ? pd.totalLaps : 0)
            const uniqueDrivers = typeof pd.unique_drivers === 'number' ? pd.unique_drivers : (typeof pd.uniqueDrivers === 'number' ? pd.uniqueDrivers : 0)
            const uniqueClasses = typeof pd.unique_classes === 'number' ? pd.unique_classes : (typeof pd.uniqueClasses === 'number' ? pd.uniqueClasses : 0)
            ...
            return {
              ...
              sessionCount,
              totalLaps,
              totalTrackTimeSeconds: typeof pd.total_track_time_seconds === 'number' ? pd.total_track_time_seconds : (typeof pd.totalTrackTimeSeconds === 'number' ? pd.totalTrackTimeSeconds : 0),
              uniqueDrivers,
              uniqueClasses,
              ...
              sessions: (pd.sessions || []).map((s: any) => ({
                ...
                durationSeconds: typeof s.duration_seconds === 'number' ? s.duration_seconds : (typeof s.durationSeconds === 'number' ? s.durationSeconds : 0),
                lapCount: typeof s.lap_count === 'number' ? s.lap_count : (typeof s.lapCount === 'number' ? s.lapCount : 0),
              })),
            }
          })
        ...
        const searchData = await parseApiResponse<{ practiceDays: IngestedPracticeDay[] }>(searchResponse)
      ...
      {isLoading && practiceDays.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          <div className="flex items-center justify-center gap-2">
            <svg className="animate-spin h-5 w-5 text-[var(--token-text-secondary)]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="text-center py-12 text-gray-400">
          <p className="mb-2">No practice days found for {selectedTrack.trackName} in {month}/{year}</p>
          <p className="text-sm text-[var(--token-text-muted)]">Try selecting a different month or track.</p>
        </div>
      ) : (
        <>
            ...
            {paginatedPracticeDays.map((practiceDay) => {
              ...
              return (
                <PracticeDayRow
                  ...
                  trackName={selectedTrack.trackName}
                  sessionCount={typeof practiceDay.sessionCount === 'number' ? practiceDay.sessionCount : 0}
                  totalLaps={typeof practiceDay.totalLaps === 'number' ? practiceDay.totalLaps : 0}
                  uniqueDrivers={typeof practiceDay.uniqueDrivers === 'number' ? practiceDay.uniqueDrivers : 0}
                  uniqueClasses={typeof practiceDay.uniqueClasses === 'number' ? practiceDay.uniqueClasses : 0}
