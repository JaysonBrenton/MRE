 ...
 * 
 * @relatedFiles
 * - src/components/event-search/EventRow.tsx (uses this component, calculates progress)
 */
 ...
 * Falls back to a default color if variable is not found
 */
function getCSSVariableValue(variableName: string, element: HTMLElement | null, fallback: string): string {
 ...
 */
function interpolateColor(color1: string, color2: string, progress: number, isBackground: boolean = false): string {
...
export default function EventStatusBadge({ status, progress, stage }: EventStatusBadgeProps) {
  ...
  React.useEffect(() => {
    if (status === "importing" && progress !== undefined && badgeRef.current) {
      ...
      const warningBgVar = computedStyle.getPropertyValue("--token-status-warning-bg").trim() || warningBgDefault
      const warningTextVar = computedStyle.getPropertyValue("--token-status-warning-text").trim() || warningTextDefault
      const successBgVar = computedStyle.getPropertyValue("--token-status-success-bg").trim() || successBgDefault
      const successTextVar = computedStyle.getPropertyValue("--token-status-success-text").trim() || successTextDefault
  ...
  const useDynamicColors = status === "importing" && progress !== undefined && dynamicColors !== null
  
  // Update label to include percentage when progress is available
  const displayLabel = status === "importing" && progress !== undefined
  ...
  const buildProgressBarStyle = (): React.CSSProperties => {
    ...
    // Get the success background color for the filled portion
    const computedStyle = badgeRef.current ? getComputedStyle(badgeRef.current) : null
    const successBgDefault = "rgba(16, 185, 129, 0.4)"
    const successBg = computedStyle?.getPropertyValue("--token-status-success-bg").trim() || successBgDefault
    
    // Increase opacity for the progress bar fill to make it more visible
    let filledColor = successBg
    if (successBg.includes("rgba")) {
      filledColor = successBg.replace(/rgba\(([^)]+)\)/, (match, values) => {
