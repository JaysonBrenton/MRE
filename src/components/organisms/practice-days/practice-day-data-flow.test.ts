...
interface PracticeDayInput {
  ...
  total_track_time_seconds?: number | null
  totalTrackTimeSeconds?: number | null
  unique_drivers?: number | null
...
interface ApiResponse {
  practiceDays?: PracticeDayInput[]
  practice_days?: PracticeDayInput[]
}

interface TransformedPracticeDay {
  date?: string
  trackSlug?: string
  sessionCount: number
  totalLaps: number
...
describe("Practice Day Data Flow - End to End", () => {
  ...
  const transformApiResponseToComponentProps = (apiResponse: ApiResponse | PracticeDayInput[]) => {
    // Step 1: Extract practice days from API response
    const practiceDaysList =
      (apiResponse as ApiResponse).practiceDays ||
      (apiResponse as ApiResponse).practice_days ||
      (Array.isArray(apiResponse) ? apiResponse : [])

    // Step 2: Transform snake_case to camelCase (from PracticeDaySearchContainer)
    const transformedPracticeDays = practiceDaysList.map((pd: PracticeDayInput) => ({
      date: pd.date,
      ...
      sessions: (pd.sessions || []).map((s: SessionInput) => ({
    ...
    return transformedPracticeDays.map((pd: TransformedPracticeDay) => {
