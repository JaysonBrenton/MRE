...
export default function OverviewTabTesting({
  ...
  if (selectedClass && unselectedDriversInClassNames.length > 0) {
    allNotices.push({
      ...
      description: (
        <>
          These drivers are in the selected class but were not included in your selection. They may
          have been manually deselected or were not included when the class was selected. You can
          select them in the driver selection panel above to include them in the chart.
        </>
      ),
  ...
  if (missingBestLapDriverNames.length > 0) {
    allNotices.push({
      ...
      description:
        "LiveRC did not publish a best lap time for these drivers in the selected class, so they are hidden from the chart.",
  ...
  if (missingAvgVsFastestDriverNames.length > 0) {
    allNotices.push({
      ...
      description:
        "These drivers were selected, but the data feed does not include both best and average lap times for them.",
      ...
      {!selectedClass && selectedDriverIds.length === 0 && (
        <div className="rounded-lg border border-[var(--token-border-default)] bg-[var(--token-surface-elevated)] px-4 py-3">
          <p className="text-sm text-[var(--token-text-secondary)]">
            <span className="font-semibold text-[var(--token-text-primary)]">Tip:</span> Select a
            class or multiple drivers above to see driver highlights below.
          </p>
        </div>
      )}
          ...
          <div>
            ...
            <div className="mt-2 rounded-md bg-[var(--token-surface-elevated)] border border-[var(--token-border-default)] px-3 py-2">
              <p className="text-xs text-[var(--token-text-secondary)]">
                💡 <span className="font-medium">Tip:</span> Click legend items to show or hide
                metrics.
              </p>
            </div>
          </div>
