                ...
                >
                  <div className="py-3 px-4">
                    <input
                      ...
                      checked={isSelected}
                      onChange={(e) =>
                        handleDriverSelectionChange(driver.driverId, e.target.checked)
                      }
                      className="w-4 h-4 rounded border-[var(--token-border-default)] text-[var(--token-accent)] focus:ring-2 focus:ring-[var(--token-interactive-focus-ring)]"
                    />
                  </div>
                  ...
                  <div className="py-3 px-4 text-[var(--token-text-secondary)]">
                    {driver.bestLapTime
                      ? `${Math.floor(driver.bestLapTime / 60)}:${(driver.bestLapTime % 60)
                          .toFixed(3)
                          .padStart(6, "0")}`
                      : "N/A"}
                  </div>
                  <div className="py-3 px-4 text-[var(--token-text-secondary)]">
                    {driver.avgLapTime
                      ? `${Math.floor(driver.avgLapTime / 60)}:${(driver.avgLapTime % 60)
                          .toFixed(3)
                          .padStart(6, "0")}`
                      : "N/A"}
                  </div>
                  <div className="py-3 px-4 text-[var(--token-text-secondary)]">
                    {driver.consistency !== null ? `${driver.consistency.toFixed(1)}%` : "N/A"}
                  </div>
                </div>
              )
            })}
