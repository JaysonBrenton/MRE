...
const driverColors = ["var(--token-accent)", "#4ecdc4", "#ff6b6b", "#ffe66d", "#a8e6cf", "#ff8b94"]
                ...
                <Group left={margin.left} top={margin.top}>
                  ...
                  {driverLapTrends.map((trend, driverIndex) => {
                    ...
                    const bestLapData = allRaceLabels
                      .map((label) => {
                        const session = trend.sessions.find((s) => s.raceLabel === label)
                      ...
                      .filter((d): d is { raceLabel: string; lapTime: number } => d !== null)
        ...
        {driverLapTrends.map((trend, index) => {
          ...
          return (
            <div key={trend.driverId} className="flex items-center gap-2">
              ...
              <span className="text-[var(--token-text-secondary)]">{trend.driverName}</span>
            </div>
          )
        })}
      </div>
