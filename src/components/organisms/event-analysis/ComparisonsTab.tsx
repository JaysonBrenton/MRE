...
export default function ComparisonsTab({ selectedClass, eventId, data }: ComparisonsTabProps) {
  ...
  const filteredRaces = useMemo(() => {
    ...
    if (filtered.length === 0 && availableRaces.length > 0) {
      console.warn(
        "[ComparisonsTab] Filtering by class resulted in 0 races, showing all races as fallback"
      )
      return availableRaces
    }
          ...
          if (raceData && raceData.laps && raceData.laps.length > 0) {
            chartData.push({
              ...
              laps: raceData.laps
                ...
                .sort(
                  (a: { lapNumber: number }, b: { lapNumber: number }) => a.lapNumber - b.lapNumber
                ),
            })
          }
        }
      ...
      {filteredRaces.length > 0 ? (
        ...
        <div className="text-center py-8">
          ...
          {data && data.races && data.races.length > 0 && filteredRaces.length === 0 && (
            <p className="text-xs text-[var(--token-text-muted)] mt-2">
              Debug: {availableRaces.length} races available, but {filteredRaces.length} after
