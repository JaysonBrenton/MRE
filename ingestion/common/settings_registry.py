"""Canonical registry of ingestion-related settings for the admin console.
...
INGESTION_SETTING_CATEGORY_ORDER: tuple[IngestionSettingCategory, ...] = (
    "scraping_safety", "ingestion_queue", "track_sync", "recent_events_auto_ingest", "practice_days", "telemetry", "infrastructure", "site_policy", "code_constants",
)
...
INGESTION_SETTINGS_REGISTRY: tuple[IngestionSettingDefinition, ...] = (
    _define(
        ...
        description="Global kill switch for all LiveRC HTTP requests (cron, CLI, downloads, admin sync).",
        category="scraping_safety",
        type="boolean",
        default=True,
        scope="both",
        apply_mode="runtime",
        confirm_when="disable_scrape",
        docker_service="all",
    ),
    _define(
        ...
        scope="ingestion",
        apply_mode="restart",
        docker_service="liverc-ingestion-service",
    ),
    _define(
    ...
    _define(
        ...
        description="When enabled, ingest requests return 202 and process in the background (forces one Uvicorn worker).",
        category="ingestion_queue",
        type="boolean",
        default=True,
        scope="ingestion",
        apply_mode="restart",
        confirm_when="disable_queue",
        docker_service="liverc-ingestion-service",
    ),
    _define(
    ...
    _define(
        ...
        description="Enables the nightly cron job that full-ingests recent events on configured tracks.",
    ...
    _define(
        ...
        scope="ingestion",
        apply_mode="runtime",
        enum_values=("followed", "active", "all"),
        docker_service="liverc-ingestion-service",
    ),
    _define(
        ...
        min=0,
        max=500,
        confirm_when="unlimited_ingests",
        docker_service="liverc-ingestion-service",
    ),
    _define(
    ...
    _define(
        ...
        scope="telemetry",
        apply_mode="restart",
        docker_service="telemetry-worker",
    ),
    _define(
    ...
    _define(
        ...
        scope="ingestion",
        apply_mode="readonly",
        enum_values=("development", "production"),
        docker_service="liverc-ingestion-service",
    ),
    _define(
    ...
    _define(
        ...
        description="Shared secret for service-to-service admin settings API (never expose to browser).",
        category="infrastructure",
        type="string",
        default="",
        scope="ingestion",
    ...
    _define(
        ...
        description="Adaptive race page fetch concurrency starting value (pipeline.py). Range 4 to 16.",
