...
def test_parse_rankings_list_extracts_qual_points(parser, event_html):
    ...
    qual_points, round_rankings, overall_final_ranking = parser.parse(event_html, url)
    ...
    assert overall_final_ranking is not None
    assert overall_final_ranking.source_overall_ranking_id == "486677"


def test_parse_rankings_list_extracts_round_rankings(parser, event_html):
    """Test that Round Ranking links are extracted."""
    url = "https://canberraoffroad.liverc.com/results/?p=view_event&id=486677"
    qual_points, round_rankings, _overall_final_ranking = parser.parse(event_html, url)
...
def test_parse_rankings_list_view_points_url_encoded_underscore(parser):
    ...
    <a href="/results/?p=event%5Foverall%5Franking&id=499602">Overall Final Ranking</a>
    </body></html>
    """
    url = "https://canberraoffroad.liverc.com/results/?p=view_event&id=499602"
    qual_points, round_rankings, overall_final_ranking = parser.parse(html, url)
    assert len(qual_points) == 1
    ...
    assert overall_final_ranking is not None
    assert overall_final_ranking.source_overall_ranking_id == "499602"
