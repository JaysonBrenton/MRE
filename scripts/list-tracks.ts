...
async function main() {
  ...
  const trackNameWidth = Math.max("Track Name".length, ...tracks.map((t) => t.trackName.length))
  const slugWidth = Math.max("Slug".length, ...tracks.map((t) => t.sourceTrackSlug.length))
  ...
  console.log(
    formatTableRow(
      [
        "-".repeat(trackNameWidth),
        "-".repeat(slugWidth),
        "-".repeat(sourceWidth),
        "-".repeat(activeWidth),
        "-".repeat(followedWidth),
        "-".repeat(eventsWidth),
        "-".repeat(lastSeenWidth),
      ],
      widths
    )
  )
  ...
  tracks.forEach((track) => {
    const lastSeen = track.lastSeenAt ? track.lastSeenAt.toISOString().split("T")[0] : "Never"
    ...
    console.log(
      formatTableRow(
        [track.trackName, track.sourceTrackSlug, track.source, active, followed, events, lastSeen],
        widths
      )
    )
  })
