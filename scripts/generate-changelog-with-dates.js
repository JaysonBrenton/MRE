...
function detectCommitType(message) {
  ...
  const conventionalMatch = message.match(/^(feat|fix|docs|style|refactor|perf|test|chore|build|ci|revert)(\(.+\))?:/i);
  ...
  for (const [type, section] of Object.entries(TYPE_MAPPINGS)) {
    if (lowerMessage.includes(type) && (lowerMessage.includes('add') || lowerMessage.includes('new') || lowerMessage.includes('create'))) {
...
function getCommits() {
  try {
    ...
    const logOutput = execSync(
      `git log --pretty=format:"${logFormat}" --date=short --all`,
    ...
    for (const line of lines) {
      ...
      if (subject.toLowerCase().includes('merge') || 
  ...
  for (let i = 0; i < sortedTags.length; i++) {
    ...
    try {
      ...
      try {
        ...
        const nextTagDate = i > 0 && sortedTags[i - 1].date 
          ? new Date(sortedTags[i - 1].date) 
          : new Date();
...
`;
    ...
    const sectionOrder = ['Added', 'Changed', 'Deprecated', 'Removed', 'Fixed', 'Security', 'Documentation', 'Testing'];
