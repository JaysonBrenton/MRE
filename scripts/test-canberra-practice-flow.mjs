...
async function main() {
  ...
  console.log(
    "   practice_days count:",
    Array.isArray(discoverJson.data?.practice_days) ? discoverJson.data.practice_days.length : "n/a"
  )
  console.log(
    "   practiceDays count:",
    Array.isArray(discoverJson.data?.practiceDays) ? discoverJson.data.practiceDays.length : "n/a"
  )
  ...
  if (Array.isArray(practiceDays) && practiceDays.length > 0) {
    console.log(
      ...
      practiceDays[0].date,
      "sessions:",
      practiceDays[0].session_count ?? practiceDays[0].session_count
    )
  }
  console.log("")
  if (!Array.isArray(practiceDays) || practiceDays.length === 0) {
